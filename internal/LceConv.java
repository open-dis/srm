...
class LceConv extends Conversions
{
    ...
    protected LceConv()
    {
        ...
        super (SRM_SRFT_Code.SRFTCOD_LOCOCENTRIC_EUCLIDEAN_3D,
    ...
    {
        return (Conversions) new LceConv();
    }
    ...
    ) throws SrmException
    {
        ...
        if (destSrfType == SRM_SRFT_Code.SRFTCOD_CELESTIOCENTRIC)
        {
            ...
            if (_toCcenConst == null)
              _toCcenConst = new ToCcenConst(((SRF_LococentricEuclidean3D)
                                              srcSrf).getSRFParameters());

            for (int i = 0; i < 3; i++)
            {
                dest[i] =
                  _toCcenConst.lococentre[i] +
                  src[0]*_toCcenConst.primary_axis[i] +
                  src[1]*_toCcenConst.secondary_axis[i] +
                  src[2]*_toCcenConst.xprod_rs[i];
            }
            dest[3] = 0.0;
        }
 ...
 */
    private class ToCcenConst
    {
        ...
        {
            Const.vectCrossProd(params.primary_axis, params.secondary_axis,
                                xprod_rs);
